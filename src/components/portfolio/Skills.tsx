          ...
          {groups.map((g, i) => (
            ...
            >
              <div className="mb-2 flex items-start justify-between gap-3">
                <h3 className="font-display text-lg font-semibold tracking-[-0.03em] md:text-xl">{g.title}</h3>
              </div>
              <div className="mt-1 flex flex-wrap items-start gap-2">
                {g.items.map((it) => (
                  <span
                    key={it}
                    className="rounded-full border border-white/10 bg-white/[0.03] px-3.5 py-1.5 text-sm text-foreground/80 transition hover:border-primary/40 hover:text-primary"
                  >
                    {it}
                  </span>
                ))}
